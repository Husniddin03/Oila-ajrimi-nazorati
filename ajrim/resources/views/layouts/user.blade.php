...
<head>
    ...
    <title>@yield('title', 'User Panel') — Oila Diagnostika</title>
    ...
    <div id="appWrapper" class="flex h-screen lg:flex">
        ...
        <aside class="sidebar w-60 flex-shrink-0 bg-fon2 border-r border-chegara flex-col overflow-y-auto h-screen lg:relative fixed -left-full top-0 transition-all duration-300 z-[1000] lg:left-0" id="sidebar">
            ...
            <div class="sb-logo p-5 pb-4 border-b border-chegara flex items-center gap-2.5">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-aksent to-red-800 flex items-center justify-center text-lg">🌿</div>
                <div>
                    <div class="font-playfair text-sm leading-tight">Oila Diagnostika</div>
                    <div class="text-[0.6rem] text-matn2 tracking-[0.06em] uppercase">User Panel</div>
                </div>
            </div>

            <!-- Navigation -->
            <nav class="sb-nav p-2 flex-1">
                <div class="text-[0.65rem] text-matn2 font-medium tracking-wider mb-2 px-2">ASOSIY</div>
                <a href="{{ route('user.dashboard') }}" class="sb-item flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('user.dashboard') ? 'bg-aksent/20 text-aksent' : 'hover:bg-fon3 text-matn' }}">
                    <span class="sb-item-icon text-base w-[18px] text-center">📊</span> Dashboard
                </a>
                <a href="{{ route('user.tests.index') }}" class="sb-item flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('user.tests.*') ? 'bg-aksent/20 text-aksent' : 'hover:bg-fon3 text-matn' }}">
                    <span class="sb-item-icon text-base w-[18px] text-center">📋</span> Testlar
                </a>
                <a href="{{ route('user.results.index') }}" class="sb-item flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('user.results.*') ? 'bg-aksent/20 text-aksent' : 'hover:bg-fon3 text-matn' }}">
                    <span class="sb-item-icon text-base w-[18px] text-center">�</span> Natijalar
                </a>
                
                <div class="text-[0.65rem] text-matn2 font-medium tracking-wider mb-2 px-2 mt-3">PROFIL</div>
                <a href="{{ route('user.profile.show') }}" class="sb-item flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-200 {{ request()->routeIs('user.profile.*') ? 'bg-aksent/20 text-aksent' : 'hover:bg-fon3 text-matn' }}">
                    <span class="sb-item-icon text-base w-[18px] text-center">👤</span> Profilim
                </a>
            </nav>
            ...
            <div class="sb-profil p-3.5 border-t border-chegara flex items-center gap-2.5">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-[0.7rem] text-aksent">Foydalanuvchi</div>
                </div>
            </div>
        ...
        <main class="main-wrap flex-1 flex flex-col min-h-screen lg:ml-0 ml-0 pt-16 lg:pt-0">
            ...
            <header class="topbar bg-karta border-b border-chegara px-6 py-4 flex items-center justify-between lg:px-6 lg:py-4">
                <div>
                    ...
                    <p class="text-sm text-matn2 mt-1">@yield('title', 'User Panel')</p>
                </div>
